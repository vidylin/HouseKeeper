...
public class SharedPreferencesManager {
   ...
   //	存商城的Url
	public void putMallUrl(Context context,String mall_url){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE);
		Editor editor = sf.edit();
		editor.putString(MALL_URL, mall_url);
		editor.commit();	
	}
	public String getMallUrl(Context context){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE); 
		return sf.getString(MALL_URL, null);
	}
	//  存开机页面跳转的Url
	public void putLogoUrl(Context context,String url){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE);
		Editor editor = sf.edit();
		editor.putString(LOGO_URL, url);
		editor.commit();
	}
	public String getLogoUrl(Context context){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE); 
		return sf.getString(LOGO_URL, null);
	}
	public void putSystemMessageIndex(Context context,String index){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE);
		Editor editor = sf.edit();
		editor.putString(SYSTEM_MESSAGE_INDEX,index);
		editor.commit();
	}
	public String getSystemMessageIndex(Context context){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE); 
		return sf.getString(SYSTEM_MESSAGE_INDEX, null);
	}
	public void putNoReadCount(Context context,int count){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL,
				context.MODE_PRIVATE);
		Editor editor = sf.edit();
		editor.putInt(NO_READ_COUNT, count);
		editor.commit();
	}
	public int  getNoReadCount(Context context){
		SharedPreferences sf = context.getSharedPreferences(SP_FILE_GWELL, context.MODE_PRIVATE); 
		return sf.getInt(NO_READ_COUNT, 0);
