...
import com.hrsst.housekeeper.common.utils.TcpClient;
import com.hrsst.housekeeper.common.utils.Utils;
import com.hrsst.housekeeper.common.utils.WifiUtils;
import com.hrsst.housekeeper.common.yoosee.FisheyeSetHandler;
import com.hrsst.housekeeper.entity.LocalDevice;
...
public class FList {
	...
	public FList() {
		...
		manager = this;
		lists = DataManager.findContactByActiveUser(AppApplication.context,
				NpcCommon.mThreeNum);
		maps.clear();
	...
	public void setType(String threeNum, int type) {
		...
		if (null != contact) {
			...
			DataManager.updateContact(AppApplication.context, contact);
		}
	}
			...
			if(wifiName.startsWith(AppConfig.Relese.APTAG)){
				...
				for(int i=0;i<apdevices.size();i++){
					...
					if(apdevices.get(i).contactId.equals(contactId)){
						...
						AppApplication.context.sendBroadcast(refresh);
						break;
					}
				}
			}
	...
	public void delete(Contact contact, int position, Handler handler) {
		...
		DataManager.deleteContactByActiveUserAndContactId(AppApplication.context,
		...
		if (contact.contactType == P2PValue.DeviceType.DOORBELL) {// 如果删除门铃则绑定标记为否，下次默认绑定报警ID
			SharedPreferencesManager.getInstance().putIsDoorbellBind(
					contact.contactId, false, AppApplication.context);
			SharedPreferencesManager.getInstance().putIsDoorBellToast(
					contact.contactId, false, AppApplication.context);
		}else if(contact.contactType == P2PValue.DeviceType.NVR){
			DataManager.deleteJAContactByActiveUserAndContactId(AppApplication.context, NpcCommon.mThreeNum, contact.contactId);
		}
		...
		AppApplication.context.sendBroadcast(refreshNearlyTell);
	}

	public void insert(Contact contact) {
	    Log.e("leleinsert", "id="+contact.contactId+"--"+"subtype="+contact.subType);
		DataManager.insertContact(AppApplication.context, contact);
		lists.add(contact);
		maps.put(contact.contactId, contact);
	...
	public synchronized void updateOnlineState() {
		...
		if (flist.size() <= 0) {
			...
			AppApplication.context.sendBroadcast(friends);
			return;
		}
	...
	public synchronized void searchLocalDevice() {
		...
		try {
			...
			ShakeManager.getInstance().setInetAddress(
					Utils.getIntentAddress(AppApplication.context));
...
//	}
	...
	private static Handler mHandler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			case ShakeManager.HANDLE_ID_RECEIVE_DEVICE_INFO:
                ...
                Log.e("leleSearch", "id="+id+"--"+"type="+type+"--"+"subType="+subType+"curVersion="+curVersion+"--"+"flag="+flag);
	...
	public void setIsConnectApWifi(String id,boolean isApWifi){
		...
		AppApplication.context.sendBroadcast(it);
	}
...
//                        FisheyeSetHandler.getInstance().sMemberWhiteList(contact.contactId, contact.contactPassword,appID);
                    }
                }
            }
        }.start();
