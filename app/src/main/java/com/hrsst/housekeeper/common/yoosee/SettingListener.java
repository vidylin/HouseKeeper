...
import com.hrsst.housekeeper.common.data.AlarmRecord;
import com.hrsst.housekeeper.common.data.Contact;
import com.hrsst.housekeeper.common.data.DataManager;
import com.hrsst.housekeeper.common.data.Message;
...
public class SettingListener implements ISetting {
	...
	public void ACK_vRetGetDefenceStates(String contactId, int msgId, int result) {
		...
		AppApplication.context.sendBroadcast(ack_Defence);
	}
	...
	public void vRetGetVideoFormatResult(int type) {
		...
		AppApplication.context.sendBroadcast(format_type);
	}
	...
	public void vRetGetVideoVolumeResult(int value) {
		...
		AppApplication.context.sendBroadcast(volume);
	}
	...
	public void vRetGetNetTypeResult(int type) {
		...
		AppApplication.context.sendBroadcast(net_type);
	}
	...
	public void vRetBindAlarmIdResult(int srcID, int result, int maxCount,
		...
		if (result == 1) {
			...
			AppApplication.context.sendBroadcast(alarmId);
		} else {
			Intent i = new Intent();
			i.putExtra("result", result);
	...
	public void vRetGetMotionResult(int state) {
		...
		AppApplication.context.sendBroadcast(motion);
	}
	...
	public void vRetGetBuzzerResult(int state) {
		...
		AppApplication.context.sendBroadcast(buzzer);
	}
	...
	public void vRetGetRecordTypeResult(int type) {
		...
		AppApplication.context.sendBroadcast(record_type);
	}
	...
	public void vRetGetRecordTimeResult(int time) {
		...
		AppApplication.context.sendBroadcast(record_time);
	}
	...
	public void vRetGetRecordPlanTimeResult(String time) {
		...
		AppApplication.context.sendBroadcast(plan_time);
	}
...
//				f.setDefenceState(contactId, Constants.DefenceState.DEFENCE_STATE_ON);
//			} else {
...
//		}
		...
		AppApplication.context.sendBroadcast(defence);
	}
	...
	public void vRetGetRemoteRecordResult(int state) {
		...
		AppApplication.context.sendBroadcast(record);
	}
...
//	}
	...
	public void vRetMessage(String contactId, String msgStr) {
		...
		DataManager.insertMessage(AppApplication.context, msg);
		Intent k = new Intent();
...
//		DataManager.insertSysMessage(AppApplication.context, sysMessage);
//		Intent i = new Intent();
...
//		AppApplication.context.sendBroadcast(k);
	...
	public void vRetCustomCmd(int contactId, int len, byte[] cmd) {
		...
		int time_interval = SharedPreferencesManager.getInstance()
				.getAlarmTimeInterval(AppApplication.context);
